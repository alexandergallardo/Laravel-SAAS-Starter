...
export default function ConnectedAccounts({ providers, hasPassword }: Props) {
    ...
    const disconnect = (provider: string) => {
        if (confirm(`Disconnect your ${provider} account?`)) {
            router.delete(`/settings/connected-accounts/${provider}`, { preserveScroll: true });
        }
    };
        ...
        <AppLayout breadcrumbs={[{ title: 'Connected Accounts', href: '/settings/connected-accounts' }]}>
            ...
            <ProfileLayout title="Connected Accounts" description="Manage your linked social login providers.">
                <div className="space-y-4">
                                ...
                                <CardHeader className="pb-3">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-3">
                                            <span className={`flex h-9 w-9 items-center justify-center rounded-lg ${config.color}`}>
                                                {config.icon}
                                            </span>
                                            <div>
                                                <CardTitle className="text-base">{config.label}</CardTitle>
                                                <CardDescription>
                                                    {connected ? `Connected as ${account?.name || account?.email || 'unknown'}` : 'Not connected'}
                                                </CardDescription>
                                            </div>
                                        </div>
                                        {connected ? (
                                ...
                                {connected && account?.email && (
                                    <CardContent className="pt-0">
                                        <p className="text-xs text-muted-foreground">
                                            Email: {account.email}
                                            {account.created_at && ` · Linked ${new Date(account.created_at).toLocaleDateString()}`}
                                        </p>
                                    </CardContent>
                                )}
                            </Card>
                    ...
                    {!hasPassword && (
                        <p className="text-xs text-muted-foreground">
                            You have no password set. You must keep at least one connected account to sign in.
                        </p>
                    )}
                </div>
            </ProfileLayout>
