...
function getMonthLabels(weeks: HeatmapDay[][]): { label: string; colIndex: number }[] {
...
export default function WorkspaceActivityHeatmap({ weeks, totalRequests, maxCount, dateRange }: Props) {
    ...
    return (
        <AdminLayout>
            ...
            <div className="space-y-6 p-6">
                <div>
                    <h1 className="text-2xl font-bold tracking-tight">API Activity Heatmap</h1>
                    <p className="text-muted-foreground mt-1 text-sm">
                        Daily API request volume across the platform — last 52 weeks.
                    </p>
                </div>
                ...
                <div className="grid gap-4 sm:grid-cols-3">
                    <Card>
                        <CardHeader className="pb-2">
                            <CardDescription>Total Requests (52w)</CardDescription>
                    ...
                    <Card>
                        <CardHeader className="pb-2">
                            ...
                            <CardTitle className="text-2xl">{maxCount.toLocaleString()}</CardTitle>
                        </CardHeader>
                    </Card>
                    <Card>
                        <CardHeader className="pb-2">
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Each cell represents one day. Hover for details.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <div className="overflow-x-auto">
                            <div className="inline-block min-w-full">
                                ...
                                <div className="mb-1 flex gap-[3px] pl-8">
                                    {weeks.map((_, i) => {
                                        const lbl = monthLabels.find((m) => m.colIndex === i);
                                        return (
                                            <div key={i} className="w-[13px] flex-shrink-0 text-[10px] text-muted-foreground">
                                                {lbl ? lbl.label : ''}
                                            </div>
                                        );
                                    })}
                                ...
                                <div className="flex gap-[3px]">
                                    ...
                                    <div className="flex flex-col gap-[3px] pr-1">
                                        {DAY_LABELS.map((d, i) => (
                                            <div key={d} className={`h-[13px] text-right text-[10px] leading-[13px] text-muted-foreground ${i % 2 === 1 ? '' : 'invisible'}`}>
                                                {d}
                                            </div>
                                        ))}
                                    </div>
                                    ...
                                    {weeks.map((week, wi) => (
                                        <div key={wi} className="flex flex-col gap-[3px]">
                                            {week.map((day) => (
                                                <div
                                                    key={day.date}
                                                    title={day.is_future ? '' : `${day.date}: ${day.count} requests`}
                                                    className={`h-[13px] w-[13px] rounded-[2px] transition-opacity ${
                                                        day.is_future ? 'opacity-0' : getColor(day.count, maxCount)
                                                    }`}
                                                />
                                            ))}
                                        </div>
